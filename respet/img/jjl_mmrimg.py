...
def cropxy(im, imsize, datain, Cnt, store_pth=''):
    ...
    if store_pth!='':
        nipet.img.mmrimg.array2nii( cim[::-1,::-1,:], B, store_pth, descrip='cropped')
...
def getinterfile_off(fmu, Cnt, Offst=np.array([0., 0., 0.])):
    ...
    #-------------------------------------------------------------------------
    mur = nipet.img.mmrimg.getmu_off(mu, Cnt)
...
def get_cylinder(Cnt, rad=25, xo=0, yo=0, unival=1, gpu_dim=False):
    ...
    if gpu_dim: imdsk = nipet.img.mmrimg.convert2dev(imdsk, Cnt)
    return imdsk
...
def mr2petAffine(datain, Cnt, fpet, fcomment='', rmsk=True, rfwhm=1.5, rthrsh=0.05, pi=50, pv=50, smof=0, smor=0):
    ...
    if rmsk:
        ...
        smoim = ndi.filters.gaussian_filter(nipet.img.mmrimg.getnii(fpet),
        ...
        nipet.img.mmrimg.array2nii( immsk[::-1,::-1,:], nipet.img.mmrimg.getnii_affine(fpet), fmsk)
...
def spm_resample(imref, imflo, m, intrp=1, dirout='', r_prefix='r_', del_ref_uncmpr=False, del_flo_uncmpr=False, del_out_uncmpr=False):
    ...
    if imref[-3:]=='.gz':
        imrefu = nipet.img.mmrimg.nii_ugzip(imref)
    else:
        imrefu = imref
    if imflo[-3:]=='.gz': 
        imflou = nipet.img.mmrimg.nii_ugzip(imflo)
    else:
        imflou = imflo
...
def obj_mumap(datain, Cnt, store=False, comment=''):
    ...
    if store:
        ...
        nipet.img.mmrimg.array2nii(mu[::-1,::-1,:], A, fout)
        mu_dct['fmu'] = fout
...
# JJL's MU-MAP
# ---------------------------------------------------------------------------------
def jjl_mumap(datain, txLUT, axLUT, Cnt, hst=[], t0=0, t1=0, faff='', fpet='', fileprefix='', fcomment='', store=False, petopt='nac'):
    '''
    ...
    * the reconstructed PET can be attenuation and scatter corrected or NOT using petopt
    ...
    if os.path.isfile(datain['hmumap']):
        ...
        hmudic = nipet.img.mmrimg.hdw_mumap(datain, [1, 2, 4], Cnt)
        muh = hmudic['im']
    ...
    # pCT file name
    fpct = os.path.join(os.path.dirname(datain['pCT']), fileprefix + fcomment + '.nii.gz')
    ...
    * the reconstructed PET can be attenuation and scatter corrected or NOT using petopt
    ...
    if not os.path.isfile(faff):
        ...
        if petopt=='qnt':
            ...
            mudic = nipet.img.mmrimg.obj_mumap(datain, Cnt)
            muo = mudic['im']
            ...
            fpet = recout.fpet
            # do the affine
            faff = nipet.img.mmrimg.mr2petAffine(datain, Cnt, fpet, fcomment=fcomment)
        elif petopt=='nac':
            # ---------------------------------------------
...
def rmumaps(datain, Cnt, t0=0, t1=0, use_stored=False):
    ...
    if os.path.isfile(datain['mumapUTE']) and use_stored:
        ...
        mudic = nipet.img.mmrimg.obj_mumap(datain, Cnt, store=True)
        muo = mudic['im']
    ...
    if os.path.isfile(datain['pCT']):
        ...
        pmudic = nipet.img.mmrimg.pct_mumap(datain, txLUT, axLUT, Cnt, faff=faff, fpet=recute.fpet, fcomment=fcomment)
        mup = pmudic['im']
...
def get_mumaps(datain, Cnt, hst={}, hparts=[1,2,4], t0=0, t1=0, use_stored = False, fcomment=''):
    ...
    if os.path.isfile(datain['hmumap']):
        ...
        hmud = nipet.img.mmrimg.hdw_mumap(datain, hparts, Cnt_)
        hmu = hmud['im']
        hA = hmud['affine']
    ...
    if os.path.isfile(datain['mumap']) and use_stored_mu:
        if (r.search(datain['mumap']) is None) and os.path.isfile(datain['pCT']):
            ...
            if not hst:
                ...
                hst = nipet.lm.mmrhist.hist(datain, txLUT, axLUT, Cnt_, t0=t0, t1=t1, use_stored=False)
            umap = nipet.img.mmrimg.pct_mumap(datain, txLUT, axLUT, Cnt_, hst=hst, fcomment=fcomment)
            omu = umap['im']
        else:
        ...
        umap = nipet.img.mmrimg.obj_mumap(datain, Cnt_)
        omu = umap['im']
    else:
